                      ...
                      bool is_next_command)
{
  ...
  */
  ...
  case COM_QUERY:
  {
    ...
    {
      if (unlikely(parser_state.init(thd, rwquery, rwquerylen)))
        break;
    }
    else
    {
    ...
    if (WSREP_ON)
    {
      if (rwflags & MYSQL_AUDIT_QUERY_REWRITE_FOR_EXECUTE)
      {
        if (wsrep_mysql_parse(thd, rwquery, rwquerylen,
                              &parser_state,
    ...
    else
    {
      ...
      if (rwflags & MYSQL_AUDIT_QUERY_REWRITE_FOR_EXECUTE)
      {
        mysql_parse(thd, rwquery, rwquerylen, &parser_state,
                    is_com_multi, is_next_command);
      }
      else
      {
    ...
    ha_maria::implicit_commit(thd, FALSE);
      ...
      */
      ...
      if (rwflags & MYSQL_AUDIT_QUERY_REWRITE_FOR_EXECUTE)
        parser_state.reset(rwquery, rwquerylen);
      else
